...
Pulling with **Shadowstride** can add some additional challenges, though. If you opt to pull with **Shadowstride** as the pulling tank, it may be more difficult to center the boss than with **Unmend**. Centering the boss at the start can be important in many fights, both so that everyone can attack the boss as soon as possible, and so that the boss won't jump around near the start, interrupting melee GCD uptime. Your GCD uptime is less important than the combined rest of your party's, so it's important not to grief your party here --- if this is causing issues positioning the boss after pull, it's probably not worth using **Shadowstride**.
